...
#BASIC_REL_PATH = "../jekyll/_site/projects-content/"
...
# applyAfterTheDeadlineCheck function
# Arguments : (Folder Path)
# Not working properly.... API returns empty results.
# ----------------------------------------------------------------------------------
def applyAfterTheDeadlineCheck(folderPath):
    pathlist = Path(folderPath).glob('**/*.markdown')
    #pathlist = Path(folderPath).glob('**/*.html')
    for path in pathlist:
        # because path is object not string
        pathContent = str(path)
        print(">>> Start " + pathContent)
        file = open(pathContent, 'r')

        # Osef car not html anymore
        soup = BeautifulSoup(file, features="html.parser")
        print(soup.get_text())

        url = "https://service.afterthedeadline.com/checkDocument"
        data = {'key': 'test55145879652356254852145698523659', 'data': soup.get_text()}
        r = requests.post(url, json=data)
        print(r.content)
        print(r.status_code)
        print(r.text)
        print(">>> End " + path_in_str)
        time.sleep(2) # little delay between each API request
...
def main(grammarBotAPIKey):
    print("travis_fold:start:grammar_checking")

    # Not anymore...
    #print("Start the grammar checking with After The Deadline...")
    #applyAfterTheDeadlineCheck(BASIC_REL_PATH2)

    print("Start the grammar checking with Grammar Bot...")
    applyGrammarBotCheck(grammarBotAPIKey, BASIC_REL_PATH2)

    ## Iterates on all the blog pages
    #applyAfterTheDeadlineCheck(BASIC_REL_PATH + BLOG_FOLDER_REL_PATH)
    print("travis_fold:end:grammar_checking")
    print("Grammar checking done.")
#enddef
